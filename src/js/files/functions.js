...
Сниппет (HTML): menu
*/
export function menuRight(windowWidth, menuWidth) {
	...
	function widowChandge(screenWidth) {
		if (screenWidth < windowWidth) {
			...
			for (let i = 0; i < menuItem.length; i++) {
				if (menuItem[i].parentElement.className !== '') {
					menuInitial[i] = menuItem[i].parentElement.className.split(' ')[0]
					console.log(menuInitial[i]);
				}
				sideNav.appendChild(menuItem[i]);
			}
		} else if (menuInitial.length !== 0) {
	...
	widowChandge(screenWidth)
	document.getElementById('mySidenav').style.width = menuWidth + 'px';
	burger.addEventListener('click', () => {
		burger.style.display = 'none';
		document.getElementById('mySidenav').style.left = (100 - ((menuWidth * 100) / screen.width)) + '%';
	})
...
export function menuLeft(windowWidth, menuWidth) {
	...
	function widowChandge(screenWidth) {
		if (screenWidth < windowWidth && menuInitial.length == 0) {
			burger.style.display = 'block';
			for (let i = 0; i < menuItem.length; i++) {
				if (menuItem[i].parentElement.className !== '') {
					menuInitial[i] = menuItem[i].parentElement.className.split(' ')[0];
		...
		} else if (screenWidth > windowWidth && menuInitial.length !== 0) {
			burger.style.display = 'none';
