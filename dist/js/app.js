(() => {
    ...
    if (videoBlock && videoController) videoController.addEventListener("click", (() => {
    ...
    const script_select = document.querySelector(".table__select");
    const tableBody = document.querySelector(".table__body");
    if (script_select) script_select.addEventListener("change", (() => {
    ...
    function menuLeft(windowWidth, menuWidth) {
        let burger = document.querySelector(".header__burger");
        let menuItem = document.querySelectorAll(".menu__item");
        let sideNav = document.querySelector(".sidenav ul");
        let menuInitial = [];
        let closeBtn = document.querySelector(".closebtn");
        let screenWidth = screen.width;
        widowChandge(screenWidth);
        window.addEventListener("resize", (() => {
            screenWidth = screen.width;
            widowChandge(screenWidth);
            document.getElementById("mySidenav").style.width = menuWidth ? "100%" : menuWidth + "px";
        }));
        function widowChandge(screenWidth) {
            if (screenWidth < windowWidth && 0 == menuInitial.length) {
                burger.style.display = "block";
                for (let i = 0; i < menuItem.length; i++) {
                    if ("" !== menuItem[i].parentElement.className) menuInitial[i] = menuItem[i].parentElement.className.split(" ")[0];
                    sideNav.appendChild(menuItem[i]);
                }
            } else if (screenWidth > windowWidth && 0 !== menuInitial.length) {
                burger.style.display = "none";
                closeBtn.style.display = "none";
                for (let i = 0; i < menuItem.length; i++) document.querySelector(`.${menuInitial[i]}`).appendChild(menuItem[i]);
    ...
    function initSliders() {
        ...
        if (document.querySelector(".slider-type_all")) new core(".slider-type_all", {
            modules: [ Pagination ],
            autoHeight: true,
            speed: 800,
            loop: true,
        ...
        if (document.querySelector(".slider-type_building")) new core(".slider-type_building", {
            modules: [ Pagination ],
            observer: true,
            loop: false,
            observeParents: true,
        ...
        if (document.querySelector(".slider-type_interior")) new core(".slider-type_interior", {
            modules: [ Pagination ],
            observer: true,
            loop: true,
            observeParents: true,
        ...
        if (document.querySelector(".apartments-categ-slider__body")) new core(".apartments-categ-slider__body", {
        ...
        scrollWatcherConstructor(items) {
            if (items.length) {
                ...
                let uniqParams = uniqArray(Array.from(items).map((function(item) {
                    return `${item.dataset.watchRoot ? item.dataset.watchRoot : null}|${item.dataset.watchMargin ? item.dataset.watchMargin : "0px"}|${item.dataset.watchThreshold ? item.dataset.watchThreshold : .1}`;
                })));
                uniqParams.forEach((uniqParam => {
                    ...
                    let groupItems = Array.from(items).filter((function(item) {
                        ...
                        let watchThreshold = item.dataset.watchThreshold ? item.dataset.watchThreshold : .1;
